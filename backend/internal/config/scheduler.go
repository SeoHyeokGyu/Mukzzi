...
func runExpiredQuestCleanup(questUsecase usecase.QuestUsecase) {
	slog.Info("만료된 퀘스트 정리 작업 시작")
	if err := questUsecase.ProcessExpiredQuests(context.Background()); err != nil {
		slog.Error("만료된 퀘스트 정리 중 오류 발생", slog.Any("error", err))
	} else {
		slog.Info("만료된 퀘스트 정리 작업 완료")
	}
}
