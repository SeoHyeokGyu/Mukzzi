...
func Paginated(c *gin.Context, data any, totalCount int64, page, limit int) {
...
// CursorPaginated 는 커서 기반 페이징 정보가 포함된 목록 응답을 전송합니다.
func CursorPaginated(c *gin.Context, data any, limit int, hasNext bool, nextCursor string) {
	c.JSON(http.StatusOK, Response{
		Success: true,
		Data:    data,
		Pagination: &Pagination{
...
func Unauthorized(c *gin.Context, code, message string, details ...any) {
	Error(c, http.StatusUnauthorized, code, message, details...)
}

// Forbidden 는 403 에러 응답을 전송합니다.
func Forbidden(c *gin.Context, code, message string, details ...any) {
	Error(c, http.StatusForbidden, code, message, details...)
}

// NotFound 는 404 에러 응답을 전송합니다.
func NotFound(c *gin.Context, code, message string, details ...any) {
