...
- 핵심 엔티티:
  - **User**: 계정 정보 (이메일, 닉네임), 포인트, 설정 및 연관 도메인 루트
  - **UserBody**: 키, 몸무게, 활동량 (기록 시마다 이력 생성)
  - **UserNutritionGoal**: 다이어트 목표, 계산된 일일 권장 섭취량 (1인당 1개)
- 비즈니스 규칙:
  - 닉네임은 2~12자 사이여야 하며, 특수문자를 제한함.
  - 사용자가 신체 정보(`UserBody`)를 등록/수정하거나 목표(`Goal`)를 변경하면, 시스템은 권장 섭취량(칼로리 및 탄단지)을 자동으로 재계산하여 `UserNutritionGoal`에 반영함.
  - `UserBody`는 수정이 아닌 추가 방식으로 관리되어, 추후 사용자의 체중 변화 이력 추적이 가능함.
  - 회원 탈퇴 시 개인 데이터는 soft delete 처리됨.
