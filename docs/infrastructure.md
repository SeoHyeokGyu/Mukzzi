...
## 로깅 및 관측 가능성 (Logging & Observability)

시스템의 이슈를 신속하게 파악하고 End-to-End 추적을 위해 다음과 같은 로깅 전략을 사용합니다.

### 1. Sonyflake 기반 Request ID 추적
- **생성 시점**: API 요청이 백엔드 미들웨어에 도착하는 즉시 Sonyflake를 이용해 64비트 고유 ID 생성.
- **전파 방식**:
  - **Context**: Go의 `context.Context`를 통해 각 레이어(Handler -> Usecase -> Repository)로 전달.
  - **Response Header**: 응답 헤더의 `X-Request-ID` 필드에 해당 ID를 포함하여 반환하여 프론트엔드와 추적 고리 연결.
- **활용**: 모든 로그 출력 시 이 ID를 포함하여, 하나의 API 요청으로 인해 발생하는 모든 행위(DB 쿼리, 외부 API 호출 등)를 단일 키로 묶어 조회.

### 2. 보안 및 성능이 강화된 상세 로깅
시스템 안정성과 보안을 위해 다음과 같은 상세 로깅 규칙을 적용합니다.

- **민감 정보 마스킹 (Masking)**: JSON 바디 내의 `password`, `access_token`, `refresh_token` 등 보안 필드는 로그 기록 시 `********`로 치환.
- **데이터 절삭 (Truncation)**: 로그 하나가 시스템 부하를 주지 않도록 요청/응답 바디의 기록 크기를 최대 **2KB**로 제한. 초과 데이터는 절삭 처리.
- **바이너리 데이터 제외**: 이미지 업로드(`multipart/form-data`) 등 바이너리 본문은 로깅에서 제외하여 디스크 및 메모리 오버헤드 방지.
- **구조화된 로깅 (Structured Logging)**: 표준 라이브러리 `slog`를 사용하여 JSON 형식으로 출력, 외부 로그 분석 도구(ELK, CloudWatch 등)와의 연동 최적화.

### 3. 로그 레벨 전략
- **INFO**: 정상적인 비즈니스 흐름 (로그인, 식사 기록 등).
- **ERROR**: 요청 처리 중 발생한 예외 상황 (Status 400 이상). 스택 트레이스 포함 권장.
- **DEBUG**: 개발 환경에서만 활성화하며 상세 내부 연산 과정 기록.
